...
# Path to a pretrained GloVe word-embedding file. The released pipeline uses the
# cached (BERT/RoBERTa-based) features under datasets/, so GloVe is NOT required and
# this can stay None. Only set it to a real glove.840B.300d.txt path if you intend to
# rebuild features from raw data via create_dataset.py.
word_emb_path = os.environ.get('GLOVE_PATH', None)
...
def get_args():
    ...
    # Evaluation split selection. With no flag the standard datasets/<DS>/{train,dev,test}.pkl
    # is used. --iid_setting / --ood_setting load the robustness splits from
    # datasets/<DS>/split_dataset_{2|7classes_1}/{train,dev,test_IID|test_OOD}.pkl,
    # and --seven_class selects the 7-class split directory (otherwise split_dataset_2).
    parser.add_argument('--iid_setting', action='store_true', help="test on the IID split (test_IID.pkl)")
    parser.add_argument('--ood_setting', action='store_true', help="test on the OOD split (test_OOD.pkl)")
    parser.add_argument('--seven_class', action='store_true', help="use the 7-class split_dataset_7classes_1 directory")
    ...
    if args.iid_setting and args.ood_setting:
        raise ValueError("Choose at most one of --iid_setting / --ood_setting.")
