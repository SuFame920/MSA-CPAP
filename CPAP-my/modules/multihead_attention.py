...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None,counterfactual_attention_type=None):
        ...
        if qkv_same:  # false
            ...
            f"q_last_dim={q.size(-1)} != embed_dim={self.embed_dim}（多半是切片边界写错）"
        ...
        ### +++
        # === 断言（就在 bmm 下面、你后面的 if attn_mask is not None: 之前/开头均可）===
        if attn_mask is not None:
            # 基本要求：只接受 key-padding mask，形状 (B,S)；B=bsz，S=src_len=k.size(1)
            assert attn_mask.dtype == torch.bool, \
                f"[MHA-MASK] expect bool, got {attn_mask.dtype}"
            assert attn_mask.device == query.device, \
                f"[MHA-MASK] device mismatch: mask={attn_mask.device} vs query={query.device}"
            assert attn_mask.dim() == 2, \
                f"[MHA-MASK] expect 2D (B,S) but got {tuple(attn_mask.shape)}"
            B_expected, S_expected = bsz, k.size(1)
            assert attn_mask.size(0) == B_expected and attn_mask.size(1) == S_expected, \
                f"[MHA-MASK] shape={tuple(attn_mask.shape)} != (B={B_expected}, S={S_expected})"
        ...
        if attn_mask is not None:  # 进
            try:
                ...
                attn_weights = attn_weights.view(self.num_heads, query.shape[1], query.shape[0], -1).masked_fill(
                                    attn_mask.to(query.device), -np.inf)
        ...
        if counterfactual_attention_type=="random":
            ...
            random_values = torch.rand(attn_weights.size(), device=query.device)
            ...
            permutation_indices = torch.randperm(bsz, device=attn_weights.device)
    ...
    #     W = weight[start:end, :]                           # [D_out, embed_dim]
    #     b = bias[start:end] if bias is not None else None  # [D_out]
    #     # 诊断断言
    #     assert input.size(-1) == self.embed_dim, \
    #         f"in_proj: input_last_dim={input.size(-1)} != embed_dim={self.embed_dim}"
    #     assert W.size(0) == (end-start) and W.size(1) == self.embed_dim, \
    #         f"W shape={tuple(W.shape)} mismatch with embed_dim={self.embed_dim}"
    #     return F.linear(input, W, b)
    def _in_proj(self, input, start=0, end=None, **kwargs):
        weight = kwargs.get('weight', self.in_proj_weight)
        bias = kwargs.get('bias', self.in_proj_bias)  
        assert isinstance(weight, torch.Tensor), f"W type={type(weight)}"
        assert (bias is None) or isinstance(bias, torch.Tensor), f"bias type={type(bias)}"
        weight = weight[start:end, :]
        if bias is not None:
            bias = bias[start:end]
        return F.linear(input, weight, bias)
    # def _in_proj(self, input, start=0, end=None, **kwargs):
    #     weight_full = kwargs.get('weight', self.in_proj_weight)
    #     bias_full   = kwargs.get('bias',   self.in_proj_bias)
    #     if end is None:
    #         end = weight_full.size(0)

    #     # —— 保持你原来的切片语义，只在返回前做“稳健化” —— 
    #     weight = weight_full[start:end, :]
    #     bias   = bias_full[start:end] if bias_full is not None else None

    #     # ★ 关键三步：连续化 + 断开底层别名 + 与输入对齐（Windows 上最稳）
    #     x = input.contiguous()
    #     W = weight.contiguous().clone().to(device=x.device, dtype=x.dtype)
    #     b = bias.contiguous().clone().to(device=x.device, dtype=x.dtype) if bias is not None else None
