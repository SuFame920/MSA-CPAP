...
class CPE(nn.Module):
    ...
    def __init__(self, hp, modal_type):
        ...
        if self.modal_type=="vision":
            ...
            self.IS_vision_encoder = SelfAttention(hp, d_in=hp.d_vin, d_model=hp.model_dim_self,
                                                   nhead=hp.num_heads_self, dim_feedforward=4*hp.model_dim_self,
                                                   dropout=hp.attn_dropout_debias, num_layers=hp.vision_debias_layers)

            self.CS_vision_encoder= CrossAttention(hp, d_modal1=hp.d_vin, d_modal2=hp.d_vin, d_model=hp.model_dim_cross, 
                                                   nhead=hp.num_heads_self, dim_feedforward=4*hp.model_dim_cross,
                                                   dropout=hp.attn_dropout_debias, num_layers=hp.vision_debias_layers)
        if self.modal_type=="audio":
            ...
            self.IS_audio_encoder = SelfAttention(hp, d_in=hp.d_ain, d_model=hp.model_dim_self, 
                                                  nhead=hp.num_heads_self, dim_feedforward=4*hp.model_dim_self,
                                                  dropout=hp.attn_dropout_debias, num_layers=hp.audio_debias_layers)
        ...
        if self.modal_type=="text":
            ...
            self.IS_text_encoder = SelfAttention(hp, d_in=hp.d_tin, d_model=hp.model_dim_self, 
                                                nhead=hp.num_heads_self, dim_feedforward=4*hp.model_dim_self,
                                                dropout=hp.attn_dropout_debias, num_layers=hp.text_debias_layers)
    ...
    def forward(self, input_modal, modal_mask): #进行L层自注意力和跨注意力的堆叠
        ...
        if self.modal_type=="audio":
            audio_local = self.IS_audio_encoder(input_modal,maskA=modal_mask) #[seq_len,bs,d_ain]
            audio_confounder_dict = self.acoustic_center.expand(bs, -1, -1).permute(1,0,2) #[seq_len,bs,dim_ain]
