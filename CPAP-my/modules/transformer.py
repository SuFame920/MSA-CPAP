...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None, mask1=None, mask2=None,whether_add_position=True,counterfactual_attention_type=None): #因果注意力的设计要注意全局Kmean无需位置编码，从而改进跨模态注意力
        """
        Args:
        ...
        for layer in self.layers:
            if x_in_k is not None and x_in_v is not None:
                x = layer(x, x_k, x_v, mask1=mask1, mask2=mask2,counterfactual_attention_type=counterfactual_attention_type)  #跨模态注意力的时候可调整是否需要反事实注意力
            else:
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, x_k=None, x_v=None, mask=None, mask1=None, mask2=None,counterfactual_attention_type=None):
        """
        Args:
        ...
        if x_k is None and x_v is None:  # 自注意？
            ...
            if mask2 is not None:
                assert mask2.dtype == torch.bool, f"[XATTN] mask2 dtype={mask2.dtype}, expect bool"
                # x_v: (S, B, D)  -> (B,S) 应与 K/V 对齐
                B_exp, S_exp = x_v.size(1), x_v.size(0)
                assert mask2.dim() == 2, f"[XATTN] expect mask2 2D (B,S) but got {tuple(mask2.shape)}"
                assert mask2.size(0) == B_exp and mask2.size(1) == S_exp, \
                    f"[XATTN] mask2 shape={tuple(mask2.shape)} != (B={B_exp}, S={S_exp})"

            x, _ = self.self_attn(query=x, key=x_k, value=x_v, attn_mask=mask2,counterfactual_attention_type=counterfactual_attention_type)
