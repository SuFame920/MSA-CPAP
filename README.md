...
- **A KMeans confounder dictionary** (`npy_folder/`, precomputed and shipped) used
  by the causal/counterfactual modules.
- **Counterfactual cross-modal attention** (text↔audio / text↔visual).
- **APF** — an adaptive probabilistic fusion module with KL-based gating.
- **Contrastive (InfoNCE) / CPC** auxiliary objectives.
...
│   ├── modules/            # encoders, attention, APF, counterfactual, InfoNCE, ...
...
| `--alpha` | `0.05` | weight of the CPC/NCE term (MOSEI: `0.4`) |
| `--beta` | `0.1` | weight of the likelihood term (MOSEI: `0.25`) |
| `--eta` | `0.3` | weight of the counterfactual term (MOSEI: `0.15`) |
...
- **KMeans 混杂字典**（`npy_folder/`，已预计算并随仓库提供），供因果 / 反事实模块使用。
- **反事实跨模态注意力**（文本↔语音 / 文本↔视觉）。
- **APF** —— 基于 KL 门控的自适应概率融合模块。
- **对比学习（InfoNCE）/ CPC** 辅助目标。
...
│   ├── solver.py           # 训练 / 评估循环
│   ├── model.py            # 完整 CPAP 模型
│   ├── modules/            # 编码器、注意力、APF、反事实、InfoNCE 等
...
| `--alpha` | `0.05` | CPC/NCE 项权重（MOSEI：`0.4`） |
| `--beta` | `0.1` | 似然项权重（MOSEI：`0.25`） |
| `--eta` | `0.3` | 反事实项权重（MOSEI：`0.15`） |
